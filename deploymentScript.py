...
class sara():
    ...
    def generate_images(self):
        file_path = './{}.zip'.format(self.unique_id)
            
        if os.path.exists(file_path):
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                zip_ref.extractall('./')
                src = './' + str(zip_ref.infolist()[0].filename) 
                dst = './' + str(self.unique_id)
                os.rename(src, dst)
                run_comm = 'python DeploymentImageGen.py' + ' ' + str(self.unique_id)
                subprocess.call(run_comm,shell=True)
